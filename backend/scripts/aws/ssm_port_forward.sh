...
The user is prompted for the name of the base environment and then a port forwarding session is opened to a bastion host
...
read -p "Base environment name (dev): " BASE_ENV_NAME

BASTION_INSTANCE_ID=$(aws ec2 describe-instances \
  --filters "Name=tag:env,Values=$BASE_ENV_NAME" \
